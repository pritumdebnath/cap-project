        ...
        <div class="card-body">
          ...
          <div>
            <img
              ...
              alt=""
              className="card-img-top attractioncardimage"
            />
          </div>
...
export default class Attractions extends Component {
  ...
  handleClick = (rating) => {
    if (rating === "3") {
      this.setState({
        filteredAttractions: this.state.attractions.filter(
          (attraction) => attraction.ratings <= 3
        ),
      });
    } else if (rating === "4") {
      this.setState({
        filteredAttractions: this.state.attractions.filter(
          (attraction) => attraction.ratings === 4
        ),
      });
    } else if (rating === "5") {
      this.setState({
        filteredAttractions: this.state.attractions.filter(
          (attraction) => attraction.ratings === 5
        ),
      });
    } else {
      ...
      <>
        ...
        <div>
          ...
          <div className="backattractions">
            ...
            <div>
              <div className="attractionscards">{this.attractionList()}</div>
            </div>
          </div>
        </div>
      </>
